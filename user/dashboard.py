...
def get_user_thoughts(user_id):
    #user_thoughts = thoughts_collection.find({'user_id': user_id})
    #return [thought['thought'] for thought in user_thoughts]
...
def get_user_thoughts_sorted(user_id):
    ...
    # user_thoughts = user_thoughts_collection.find().sort('datetime', DESCENDING)
...
def save_user_thought(user_id, thought , datetime_obj):
    # Encrypt the thought before saving
    ...
    user_thoughts_collection = db[f'user_{user_id}_thoughts']  # Create or get user-specific collection
    # user_thoughts_collection.insert_one({'user_id': user_id, 'thought': thought, 'datetime': datetime_obj})
    user_thoughts_collection.insert_one(thought_data)

def update_user_thought(user_id, thought_id, new_thought):
    #thoughts_collection.update_one({'user_id': user_id, '_id': ObjectId(thought_id)}, {'$set': {'thought': new_thought}})
    user_thoughts_collection = db[f'user_{user_id}_thoughts']  # Get user-specific collection
...
def convert_thoughts_to_excel(thoughts, username):
    ...
    # Create a DataFrame from the thoughts data
    df = pd.DataFrame(thoughts)
    
    # Remove _id and user_id fields
